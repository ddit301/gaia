...
WITH PROJECTINFO AS (
        ...
        ASSIGNEES AS (
            SELECT A.ISSUE_SID AS ASSIGNEES_SID, C.MEM_NO AS ASSIGNEE, C.MEM_NICK AS ASSIGNEE_NICK
            FROM ISSUE_ASSIGNEE A
            INNER JOIN MEMBER C ON (A.MEM_NO = C.MEM_NO)
        )
        
		SELECT 
		        MEMBER.MEM_NICK,MEMBER.MEM_ID, MEMBER.MEM_NO AS MEM_NO, MEMBER.MEM_BIO, MEMBER.MEM_PIC_FILE_NAME,
		        PROJECTINFO.PROJ_NO AS PROJECT_NO, PROJ_TITLE, PROJ_MANAGER_NO, PROJ_MANAGER_NICK,
		        PROJ_MANAGER_NICK||'/'||PROJ_TITLE AS PROJECT_URI,
		        PROJ_MANAGER_NICK||'/'||PROJ_TITLE||'/issue/'||ISSUE_NO AS ISSUE_URL,
		        ISSUE_SID AS ISSUE_ID, ISSUE_TITLE, FIXER_NO, FIXER_NICK, ISSUE_HIS_CONT, ISSUE_HIS_DATE, ISSUE_HIS_NO,
		        LABEL_NO, LABEL_NM, ASSIGNEES.ASSIGNEE, ASSIGNEES.ASSIGNEE_NICK
		FROM MEMBER
		        LEFT OUTER JOIN PROJECTINFO ON(MEMBER.MEM_NO = PROJECTINFO.MEM_NO)
		        LEFT OUTER JOIN ISSUEINFO ON(PROJECTINFO.PROJ_NO = ISSUEINFO.PROJ_NO AND MEMBER.MEM_NO = ISSUEINFO.ASSIGNEE)
                LEFT OUTER JOIN ASSIGNEES ON(ISSUEINFO.ISSUE_SID = ASSIGNEES.ASSIGNEES_SID)
				
		WHERE MEMBER.MEM_NO = 4
		ORDER BY ISSUE_HIS_NO DESC
        
        SELECT A.MEM_NO,A.ISSUE_SID, B.ISSUE_SID, B.MEM_NO AS ASSIGNEES, C.MEM_NICK AS ASSIGNEES_NM
        FROM ISSUE_ASSIGNEE A
        RIGHT OUTER JOIN ISSUE_ASSIGNEE B ON(A.ISSUE_SID = B.ISSUE_SID)
	...
	WITH MEMSTAT AS (
		...
		FROM MEMBER
		INNER JOIN MEMSTAT ON (MEM_STATUS = COM_CODE)
		...
		SELECT 
		        MEMBER.MEM_NICK,MEMBER.MEM_ID, MEMBER.MEM_NO AS MEM_NO, 
		        MEMBER.MEM_BIO, MEMBER.MEM_PIC_FILE_NAME, MEMSTAT.COM_CODE_NM AS MEM_STATUS,
		        PROJECTINFO.PROJ_NO AS PROJECT_NO, PROJ_TITLE, PROJ_MANAGER_NO, PROJ_MANAGER_NICK,
		        PROJ_MANAGER_NICK||'/'||PROJ_TITLE AS PROJECT_URI,
		        PROJ_MANAGER_NICK||'/'||PROJ_TITLE||'/issue/'||ISSUE_NO AS ISSUE_URL,
		        ISSUE_SID AS ISSUE_ID, ISSUE_TITLE, FIXER_NO, FIXER_NICK, ISSUE_HIS_CONT, ISSUE_HIS_DATE, ISSUE_HIS_NO,
