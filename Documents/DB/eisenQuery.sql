...
SELECT
    ISSUE.ISSUE_SID, ISSUE_HIS_CONT, ISSUE_HIS_DATE, ISSUE_HIS_TYPE, ISSUE_HISTORY.MEM_NO AS 수정자,
    ISSUE.PROJ_NO, ISSUE.MEM_NO AS 담당자
FROM
    ISSUE
    INNER JOIN ISSUE_HISTORY ON(ISSUE.ISSUE_SID = ISSUE_HISTORY.ISSUE_SID)
    INNER JOIN ISSUE_ASSIGNEE ON(ISSUE.ISSUE_SID = ISSUE_ASSIGNEE.ISSUE_SID)
)

SELECT MEM_NICK, PROJECTINFO.PROJ_NO, PROJ_TITLE, ISSUE_NO, ISSUE_TITLE
FROM MEMBER
    LEFT OUTER JOIN PROJECTINFO ON(MEMBER.MEM_NO = PROJECTINFO.MEM_NO)
    LEFT OUTER JOIN ISSUE ON(PROJECTINFO.PROJ_NO = ISSUE.PROJ_NO)
